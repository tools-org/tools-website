                ...
                {matched.groups &&
                  matched.groups.map((group: any) => {
                    return (
                      <div
                        key={group.key}
                      >{`Group ${group.key} : ${group.value} `}</div>
                    );
                  })}
              </div>
            }
    ...
    <div>
      <div className={"tools-regexp-header"}>
        <Input
          ...
          addonAfter={
            <span style={{ fontSize: 18, padding: "0 6px" }}>/{flag}</span>
          }
        />
        <Dropdown
          trigger={["click"]}
        ...
        <Dropdown
          trigger={["click"]}
          menu={{ items: REGEXP_SYNTAX_COMMENTS_OPTIONS }}
        >
          <Button
...
export default ToolModule(Regexp, TOOLS_KEY_ENUM.Regexp);
