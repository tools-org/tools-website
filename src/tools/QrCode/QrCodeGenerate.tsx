...
const QrCodeGenerate = () => {
  ...
  return (
    <div id="qrcode" className={"tools-qrcode-generate"}>
      <div className={"tools-qrcode-generate-content"}>
        <div className={"tools-qrcode-generate-code"}>
          <QRCode
        ...
        <TextArea
          rows={10}
          placeholder="请输入地址或文本内容"
          onChange={handleValueChange}
        />
      </div>
      <div className={"tools-qrcode-generate-config"}>
        <Form labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
          ...
          <FormItem
            label="容错率"
            tooltip="容错率设置越高，二维码生成的码点密度越高，可在遮挡越多的情况下被扫描出来。"
          >
            <Segmented
              options={ERROR_LEVEL}
              value={errorLevel}
          ...
          {icon ? (
            <FormItem label="icon尺寸">
              <InputNumber
                addonAfter="px"
                value={iconSize}
                onChange={(v) => setIconSize(v as number)}
              />
            </FormItem>
          ) : null}
          <Form.Item
...
export default ToolModule(QrCodeGenerate, TOOLS_KEY_ENUM.QrCodeGenerate);
